...
describe 'Administrador consulta o saldo de um cliente' do
  ...
  it 'com sucesso e o cliente possui saldo bônus' do
    admin = create(:admin, :approved)
    er = create(:exchange_rate, :approved, admin: admin)
    company = create(:company)
    ...
    create(:credit, real_amount: 500, exchange_rate: er, client: client, company: company)
    credit = create(:credit, real_amount: 650, exchange_rate: er, client: client, company: company)
    create(:bonus_credit, credit: credit, client: client, amount: 17.25)
    ...
    expect(page).to have_content(client.name)
    expect(page).to have_content('Saldo: RB 115.0 (R$ 1.150,00)')
    expect(page).to have_content('Saldo Bônus: RB 17.25')
  end
